...
public class AreaGrid<T> {
	/**
	 * Represents an area inside an AreaGrid. Say an AreaGrid is 64x64, split
	 * into 16x16 cubes. Then the AreaGrid would have 4x4 Grids inside it, each
	 * of 16x16 dimensions.
	 * 
	 * @author netherfoam
	 */
	 ...
	 * 
	 * @param width
	 *            the width of the area grid (X, or Dimension 0). RS max X
	 *            coordinate is 16383 for rev 637
	 * @param length
	 *            the length of the grid (Y, or Dimension 1). RS max Y
	 *            coordinate is 16383 for rev 637
	 * @param subgridSize
	 *            the size to subdivide the map into. Higher values cost less
	 *            RAM and more CPU time (to lookup). Lower values are more RAM
	 *            intensive, but less intensive on the CPU. Recommended is
	 *            8/16/32, depending on density and total size of map.
	 */
	...
	/**
	 * Trims all of the array lists to size, and removes empty ones from the
	 * AreaGrid. This tries to same memory.
	 */
	public void trim() {
	 ...
	 * 
	 * @param query
	 *            The MBR to check for overlaps with
	 * @param guess
	 *            The normal maximum number of results to expect (Efficiency)
	...
	/**
	 * Queries the AreaGrid for all MBR's which are instances of the given class
	 * and overlap the given query. For example, HashSet<Player> players =
	 * area.get(overlap, 256, Player.class); would be a useful call to fetch all
	 * players that are inside the MBR 'overlap'.
	 * 
	 * @param query
	 *            the MBR which is being used to query
	 * @param guess
	 *            the estimate number of results that will be found
	 * @param clazz
	 *            the type of objects you are interested in
	 * @return a HashSet of the MBR's which extend the given class and are in
	 *         the given area.
	 */
	@SuppressWarnings("unchecked")
