...
public class MBRUtil {
	public static boolean isOverlap(MBR m1, MBR m2) {
		...
		for (int i = 0; i < m1.getDimensions(); i++) {
			if (m1.getMin(i) + m1.getDimension(i) < m2.getMin(i))
				return false; //m1's biggest edge is less than m2's smallest edge.
			if (m2.getMin(i) + m2.getDimension(i) < m1.getMin(i))
				return false; //m2's biggest edge is less than m1's smallest edge.
		}
	...
	public static boolean isEqual(MBR m1, MBR m2, int numDims) {
		for (int axis = 0; axis < numDims; axis++) {
			if (m1.getDimension(axis) != m2.getDimension(axis))
				return false;
			if (m1.getMin(axis) != m2.getMin(axis))
				return false;
		}
		return true;
	}
	...
	private static void assertDims(MBR... mbr) {
		...
		for (int i = 1; i < mbr.length; i++) {
			if (mbr[i].getDimensions() != dims)
				throw new IllegalArgumentException("MBR's must have the same number of dimensions!");
		}
	}
