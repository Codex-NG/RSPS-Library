...
public class ActionQueue extends Tickable {
	 ...
	 * 
	 * @param owner
	 *            the mob who this queue will belong to.
	 * @throws NullPointerException
	 *             if owner is null
	 */
	public ActionQueue(Actor owner) {
		if (owner == null)
			throw new NullPointerException("ActionQueue owner mob may not be null.");
		this.owner = owner;
	}
	 ...
	 * 
	 * @param w
	 *            the Action to queue for the Mob
	 * @throws IllegalArgumentException
	 *             if the given action is already in the queue
	 * @throws NullPointerException
	...
	/**
	 * Fetches the first action in this ActionQueue that is a class or subclass
	 * of the given type.
	 * 
	 * @param <T>
	 *            the Action type, eg CombatFollow.class
	 * @param type
	 *            the Action type, eg CombatFollow.class
	 ...
	 * 
	 * @param type
	 *            the type to cancel, eg WalkAction
	 ...
	 * @param insert
	 *            the action to place immediately before the marker
	 * @throws IllegalArgumentException
	 *             if the insert Action is already queued
	 * @throws IllegalStateException
	 *             if the marker is not in the queue.
	 */
	 ...
	 * 
	 * @param marker
	 *            the action in the queue which wants the action after it
	 * @return the Action after the given action, or null if there is none or
	 *         the marker wasn't found
	 */
	 ...
	 * 
	 * @param marker
	 *            the action in the queue which wants the action before it
	 * @return the Action before the given action, or null if there is none or
	 *         the marker wasn't found
	 */
	...
	/**
	 * Inserts the given action at the start of the queue, may interrupt another
	 * action but does not cancel the other action. Eg use for equipping items.
	 * 
	 * @param insert
	 ...
	 *            the action to look for
	 * @param insert
	 *            the action to place immediately after the marker
	 * @throws IllegalArgumentException
	 *             if the insert Action is already queued
	 * @throws IllegalStateException
	 *             if the marker is not in the queue.
	...
	/**
	 * Gracefully ends the action - Called when the action has finished. This
	 * removes it from the queue of actions to run, and thus it will not be run
	 * again. This is different to cancelling a task, which will inform the task
	 * that it was cancelled forcefully. If you wish to cancel an action, use
	 * cancel. If your action has exited its run() method, this method will be
	 * called for you by the Fiber which ran your action.
	 * 
	 * @param a
	 *            the action which is ending
	 */
	 ...
	 * 
	 * @param yielder
	 *            the Action that is yielding. This is required to find the
	...
	protected void yield(Action yielder) {
		synchronized (queue) {
			...
			if (index == -1)
				return; //Cannot yield, since it is not queued
